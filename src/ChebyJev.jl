...
"""
    qcorootmatrix(Type::Symbol, n::Int)

Return the matrix whose rows are the simple coroots of the root system
of type `Type` and rank `n`.

This matrix is the left inverse of `qweightmatrix(Type,n)`.
"""
function qcorootmatrix(Type::Symbol, n::Int)

    B = qbasematrix(Type,n)
    H = hcat([coroot(B[:,i]) for i in 1:n]...)

    return transpose(matrix(QQ, H))
...
"""
    ...
    orbit(Type::Symbol, alpha::Vector{Int64})

Return the orbit of `omega` under the Weyl group generated by
qreflections in the simple roots.

The output is a matrix whose columns are the orbit elements.
"""
function orbit(Type::Symbol, n::Int, omega::AbstractVector{<:QQFieldElem})
    ...
    return matrix(QQ, hcat(collect(orb)...))
...
function chebyshevpolynomial(Type::Symbol, n, mu)
    W = qcorootmatrix(Type,n)
    muu = W*mu
    alpha = Vector{Int64}(undef, n)
    for i in 1:n
        @assert denominator((muu)[i]) == 1
        alpha[i] = Int64((muu)[i])
    end

    return chebyshevpolynomial(Type,alpha)
...
function multiplicative_normal_form(f::QQMPolyRingElem)
    R = parent(f)
    vars = gens(R)
    n = div(length(vars),2)
    out = R(0)
    ...
    for (m,c) in zip(mons, coeffs)
        ...
        alpha = [Int(e[i] - e[n+i]) for i in 1:n]
        m_normal = prod(alpha[i] >= 0 ? vars[i]^alpha[i] : vars[n+i]^(-alpha[i]) for i in 1:n)
        out += QQ(c) * m_normal
    end
...
function fundinvcontext(n::Int)
    ...
    if haskey(FUNDINV_CONTEXT, n) && length(FUNDINV_CONTEXT[n]) == 4
    ...
    xA = vars[1:n]
    yA = vars[n+1:2*n]
    normal_form = multiplicative_normal_form

    FUNDINV_CONTEXT[n] = (R,xA,yA,normal_form)
...
function fundamentalinvariant(Type::Symbol, n::Int)
    R, xA, yA, normal_form = fundinvcontext(n)

    if Type == :A
    L = [xA[1]; [xA[k]*yA[k-1] for k in 2:n]; yA[n]]
    return [normal_form((1//binomial(n+1,l)) * esp(L,l)) for l in 1:n]

    elseif Type == :C
        L = [xA[1]+yA[1]; [xA[k]*yA[k-1] + yA[k]*xA[k-1] for k in 2:n]]
        return [normal_form((1//(binomial(n,l)*2^l)) * esp(L,l)) for l in 1:n]

    elseif Type == :B
        L = [xA[1]+yA[1]; [xA[k]*yA[k-1] + yA[k]*xA[k-1] for k in 2:n-1]; xA[n]^2*yA[n-1] + yA[n]^2*xA[n-1]]
        orb = orbit(Type, vcat(zeros(Int64,n-1), [1]))
        orb_terms = [prod((v[i] >= 0 ? xA[i]^v[i] : yA[i]^(-v[i])) for i in 1:n) for v in eachcol(orb)]
        return vcat(
            [normal_form((1//(binomial(n,l)*2^l)) * esp(L,l)) for l in 1:n-1],
            [normal_form((1//2^n) * sum(orb_terms))]
        )
    ...
    elseif Type == :D
        ...
        orb_terms1 = [prod(v[i] >= 0 ? xA[i]^v[i] : yA[i]^(-v[i]) for i in 1:n) for v in eachcol(orb1)]
        orb_terms2 = [prod(v[i] >= 0 ? xA[i]^v[i] : yA[i]^(-v[i]) for i in 1:n) for v in eachcol(orb2)]

        return vcat(
            [normal_form((1//(binomial(n,l)*2^l)) * esp(L,l)) for l in 1:n-2],
            [normal_form((1//2^(n-1)) * sum(orb_terms1))],
            [normal_form((1//2^(n-1)) * sum(orb_terms2))]
        )
    ...
    return [normal_form(T1),normal_form(T2),normal_form(T3),normal_form(T4)]
    ...
    elseif Type == :E && 6 <= n <= 8
        ...
        for i in 1:n
            ...
            s = (1//ord) * sum(
                ...
                for v in eachcol(orb)
            )
            push!(invariants,normal_form(s))
        end
        return invariants
    ...
    return [normal_form(T1),normal_form(T2)]
...
function is_multiplicative_invariant(Type::Symbol, f::QQMPolyRingElem)
    ...
    f_normal = multiplicative_normal_form(f)

    for s in W
        f_image = multiplicative_action(Type, s, f_normal)
        if f_normal != f_image
            return false
        end
    end
...
function hermite_entry_z(Type::Symbol, n::Int, k::Int)
    return t_to_z_basis(Type, hermite_entry_T(Type, n, k))
end

function hermite_matrix_z(Type::Symbol, n::Int)
    MT = hermite_matrix_T(Type, n)

    return [t_to_z_basis(Type, MT[i,j]) for i in 1:size(MT,1), j in 1:size(MT,2)]
end
